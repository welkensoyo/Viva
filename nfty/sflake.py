...
class API:
    ...
    def charts(self):
        ...
        nonnurses = q.format('''AND s.S_ACTUAL_END < DATEADD(DAY, -1, DATE_TRUNC('MONTH', CURRENT_DATE)) AND c.CG_DISCIPLINENAME NOT IN ('RN','LVN') ''').replace(", CAST(DATEADD(DAY, -1 - EXTRACT(DAYOFWEEK FROM s.SCHEDULE_DATE), DATE_TRUNC('DAY', s.SCHEDULE_DATE)) as VARCHAR)", '').replace(' AS WEEK_OF', '')
        nurses = q.format('''AND s.S_ACTUAL_END < DATEADD(DAY, -1, DATE_TRUNC('MONTH', CURRENT_DATE)) AND c.CG_DISCIPLINENAME IN ('RN','LVN') ''').replace(", CAST(DATEADD(DAY, -1 - EXTRACT(DAYOFWEEK FROM s.SCHEDULE_DATE), DATE_TRUNC('DAY', s.SCHEDULE_DATE)) as VARCHAR)", '').replace(' AS WEEK_OF', '')
