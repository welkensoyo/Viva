...
SELECT YEAR(s.SCHEDULE_DATE) as YEAR, MONTH(S.SCHEDULE_DATE) AS MONTH, TO_CHAR(s.SCHEDULE_DATE,'MON') AS MONTH_ABBR,
    h.AGENCY_BRANCH_NAME, c.CG_EMPLOYEEID AS USERID, CONCAT(c.CG_FIRSTNAME,' ', c.CG_LASTNAME) AS "CLINICIAN NAME", c.CG_DISCIPLINENAME as DISCIPLINE,
     CASE 
...
GROUP BY c.CG_EMPLOYEEID, CONCAT(c.CG_FIRSTNAME,' ', c.CG_LASTNAME), c.CG_DISCIPLINENAME, TO_CHAR(s.SCHEDULE_DATE,'MON'), YEAR(s.SCHEDULE_DATE), MONTH(S.SCHEDULE_DATE), sc.SEVICE_CODE, h.AGENCY_BRANCH_NAME
ORDER BY 3 DESC;''',
...
WITH Weekly_Summary AS (
    ...
    FROM
        KANTIME_PROD_DB.HH_REPORT_DS.SCHEDULEMASTER_SVW AS s
    GROUP BY
        s.S_CLIENT_ID,
        s.S_CAREGIVER_ID,
    ...
    SELECT
        ...
        (
            ...
            WHERE
                ...
                AND ws2.WEEK_END >= DATEADD('day', -60, ws1.WEEK_END) -- Last 60 days relative to ws1.WEEK_END
...
SELECT YEAR(s.SCHEDULE_DATE) as YEAR, MONTH(S.SCHEDULE_DATE) AS MONTH, TO_CHAR(s.SCHEDULE_DATE,'MON') AS MONTH_ABBR,
    DATEADD('day', 6 - IFF(DAYOFWEEK(s.SCHEDULE_DATE) = 7, 0, DAYOFWEEK(s.SCHEDULE_DATE)), s.SCHEDULE_DATE) AS WEEK_END,
    s.SCHEDULE_DATE,
    h.AGENCY_BRANCH_NAME, u.CLIENT_ID, CONCAT(u.CLIENT_FIRST_NAME, ' ',u.CLIENT_LAST_NAME) as "CLIENT NAME",
    c.CG_EMPLOYEEID AS CG_ID, CONCAT(c.CG_FIRSTNAME,' ', c.CG_LASTNAME)  AS "CLINICIAN NAME",
    ...
    a.AUTH_IS_VISITBASED as AUTH_VISIT_BASED,
    ...
    a.AUTH_VISIT_COUNT  as AUTH_VISIT_COUNT,
...
LEFT JOIN KANTIME_PROD_DB.HH_REPORT_DS.AUTHORIZATIONMASTER_SVW as a ON a.AUTH_ID = s.S_AUTHORIZATION_ID
LEFT JOIN Rolling_60_Day_Summary as w ON w.S_CAREGIVER_ID = s.S_CAREGIVER_ID AND w.S_CLIENT_ID = u.CLIENT_ID AND DATEADD('day', 6 - IFF(DAYOFWEEK(s.SCHEDULE_DATE) = 7, 0, DAYOFWEEK(s.SCHEDULE_DATE)), s.SCHEDULE_DATE) = w.WEEK_END
WHERE s.S_WEEKSTART >= DATEADD('month', -24, CURRENT_DATE())
{}
...
DATEADD('day', 6 - IFF(DAYOFWEEK(s.SCHEDULE_DATE) = 7, 0, DAYOFWEEK(s.SCHEDULE_DATE)), s.SCHEDULE_DATE),s.SCHEDULE_DATE,
YEAR(s.SCHEDULE_DATE), MONTH(S.SCHEDULE_DATE), TO_CHAR(s.SCHEDULE_DATE,'MON'), u.CLIENT_ID, CONCAT(u.CLIENT_FIRST_NAME, ' ',u.CLIENT_LAST_NAME),
...
a.AUTH_IS_VISITBASED, a.AUTH_VISIT_COUNT
ORDER BY 4 DESC;''',
...
JOIN KANTIME_PROD_DB.HH_REPORT_DS.CAREGIVERMASTER_SVW as c ON s.S_CAREGIVER_ID = c.CG_EMPLOYEEID
JOIN KANTIME_PROD_DB.HH_REPORT_DS.HOMEHEALTHAGENCIESBRANCHLIST_SVW as h ON c.CG_PAYROLL_BRANCH_ID = h.AGENCY_BRANCH_ID
WHERE (s.S_ACTUAL_END IS NOT NULL OR s.S_SCHEDULE_STATUS = 'MissedVisit') AND s.S_WEEKSTART >= DATEADD('month', -24, CURRENT_DATE())
...
GROUP BY c.CG_EMPLOYEEID, CONCAT(c.CG_FIRSTNAME,' ', c.CG_LASTNAME), c.CG_DISCIPLINENAME, sc.SEVICE_CODE, h.AGENCY_BRANCH_NAME,
...
ORDER BY 1 DESC, 2 DESC''',
    ...
    SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE))), s.S_ACTUAL_HOURS, NULL)) as HOURS_THIS_YEAR,
    ROUND(SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE))) AND s.IS_SCHEDULE_BILLED, s.S_BILLED_AMOUNT, NULL)),2) AS BILLED_THIS_YEAR,
    ROUND(SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE))) AND NOT s.IS_SCHEDULE_BILLED, s.S_UNITS * S.S_BILL_RATE, NULL)),2) AS UN_BILLED_THIS_YEAR,
    ROUND(SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE))), s.S_UNITS * S.S_BILL_RATE, NULL)),2) AS TOTAL_THIS_YEAR,
    ROUND(SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE))), S.S_OT_PAYROLLAMOUNT, NULL))) as OT_THIS_YEAR,
    ROUND(SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE))), S.S_OT_HOURS, NULL))) as OT_HOURS_THIS_YEAR,
    ROUND((SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE))), s.S_OT_HOURS, NULL)) / NULLIF(SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE))) AND s.IS_SCHEDULE_BILLED, s.S_ACTUAL_HOURS, NULL)), 0)) * 100, 2) AS "OT_%_THIS_YEAR",

    -- Sum values for the last year
    SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE)))-1, s.S_ACTUAL_HOURS, NULL)) as HOURS_LAST_YEAR,
    ROUND(SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE)))-1 AND s.IS_SCHEDULE_BILLED, s.S_BILLED_AMOUNT, NULL)),2) AS BILLED_LAST_YEAR,
    ROUND(SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE)))-1 AND NOT s.IS_SCHEDULE_BILLED, s.S_UNITS * s.S_BILL_RATE, NULL)),2) AS UN_BILLED_LAST_YEAR,
    ROUND(SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE)))-1, s.S_UNITS * S.S_BILL_RATE, NULL)),2) AS TOTAL_LAST_YEAR,
    ROUND(SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE)))-1, S.S_OT_PAYROLLAMOUNT, NULL))) as OT_LAST_YEAR,
    ROUND(SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE)))-1, S.S_OT_HOURS, NULL))) as OT_HOURS_LAST_YEAR,
    ROUND((SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE)))-1, s.S_OT_HOURS, NULL)) / NULLIF(SUM(IFF(YEAR(s.SCHEDULE_DATE) = YEAR(LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE)))-1 AND s.IS_SCHEDULE_BILLED, s.S_ACTUAL_HOURS, NULL)), 0)) * 100, 2) AS "OT_%_LAST_YEAR"
...
AND s.S_ACTUAL_END >= DATEADD('year', -2, DATE_TRUNC('year', DATEADD('month', -1, LAST_DAY(DATEADD(MONTH, -1, CURRENT_DATE)))))
{}
...
ORDER BY 4 DESC;''',
    ...
    'weekly_hours': '''SELECT  DATEADD('day', 6 - IFF(DAYOFWEEK(s.SCHEDULE_DATE) = 7, 0, DAYOFWEEK(s.SCHEDULE_DATE)), s.SCHEDULE_DATE) AS WEEK_END,
    h.AGENCY_BRANCH_NAME,
    SUM(ROUND(s.S_ACTUAL_HOURS / 0.25, 0) * 0.25) as ACTUAL_HOURS
FROM KANTIME_PROD_DB.HH_REPORT_DS.SCHEDULEMASTER_SVW AS s
JOIN KANTIME_PROD_DB.HH_REPORT_DS.SERVICECODESMASTER_SVW as sc on sc.SERVICE_ID = s.S_SERVICE_CODE_ID
...
GROUP BY
DATEADD('day', 6 - IFF(DAYOFWEEK(s.SCHEDULE_DATE) = 7, 0, DAYOFWEEK(s.SCHEDULE_DATE)), s.SCHEDULE_DATE), h.AGENCY_BRANCH_NAME''',
...
FROM
   ...
   with cte1 (
        ...
        select
            ...
            SCHEDULE_DATE,
            DATEADD('day', 6 - IFF(DAYOFWEEK(SCHEDULE_DATE) = 7, 0, DAYOFWEEK(SCHEDULE_DATE)), SCHEDULE_DATE) AS "WK_END",
            SC.SERVICE_TYPE,
            SC.SERVICE_NAME,
            SM.S_REGULAR_HOURS,
            SM.S_PAY_RATE,
        ...
        from KANTIME_PROD_DB.HH_REPORT_DS.CLIENTMASTER_SVW CM
    ...
    cte2 as (
        select
            ...
            cte1.SCHEDULE_DATE,
            DATEADD('day', 6 - IFF(DAYOFWEEK(cte1.SCHEDULE_DATE) = 7, 0, DAYOFWEEK(cte1.SCHEDULE_DATE)), cte1.SCHEDULE_DATE) AS "WK_END",
            cte1.DISCIPLINE,
            cte1.SERVICE_NAME,
            CASE
...
SELECT
    ...
    SUM(IFF(YEAR(s.SCHEDULE_DATE) >= YEAR(c.CG_HIREDDATE) AND s.S_ACTUAL_END IS NOT NULL AND s.S_SCHEDULE_STATUS = 'Approved', s.S_ACTUAL_HOURS, NULL)) as TOTAL_HOURS,
    SUM(IFF(s.SCHEDULE_DATE >= DATEADD(month, -6, CURRENT_DATE()) AND s.S_ACTUAL_END IS NOT NULL AND s.S_ACTUAL_HOURS AND s.S_SCHEDULE_STATUS = 'Approved', s.S_ACTUAL_HOURS, NULL)) as HOURS_LAST_SIX_MONTHS,
...
LEFT JOIN KANTIME_PROD_DB.HH_REPORT_DS.SCHEDULEMASTER_SVW AS s ON c.CG_EMPLOYEEID = s.S_CAREGIVER_ID AND s.S_UNITS > 0
...
WHERE
    ...
    AND s.S_WEEKSTART >= DATEADD('month', -24, CURRENT_DATE())
GROUP BY
    c.CG_EMPLOYEEID, CONCAT(c.CG_FIRSTNAME,' ', c.CG_LASTNAME), c.CG_DISCIPLINENAME,
