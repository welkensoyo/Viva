...
SELECT YEAR(s.SCHEDULE_DATE) as YEAR, MONTH(S.SCHEDULE_DATE) AS MONTH, TO_CHAR(s.SCHEDULE_DATE,'MON') AS MONTH_ABBR,
    ...
    SUM(s.S_ACTUAL_HOURS) as APPROVED_HOURS,
    sc.SERVICE_TYPE,
    s.S_SCHEDULE_STATUS,
     CASE 
        WHEN TRIM(sc.SEVICE_CODE) IN ('PDN RN HITech', 'PDN RN HiTech', 'PDN RN HiTECH', 'PDN Hi Tech - RN') THEN 'PDN RN HI TECH'
    ...
    ROUND(SUM(IFF(s.IS_SCHEDULE_BILLED, s.S_UNITS * S.S_BILL_RATE, 0)),2) AS BILLED,
...
ORDER BY 4 DESC;''',
   'patient_metrics' : '''
    SELECT YEAR(s.SCHEDULE_DATE) as YEAR, MONTH(S.SCHEDULE_DATE) AS MONTH, TO_CHAR(s.SCHEDULE_DATE,'MON') AS MONTH_ABBR,
    DATEADD('day', 6 - IFF(DAYOFWEEK(s.SCHEDULE_DATE) = 7, 0, DAYOFWEEK(s.SCHEDULE_DATE)), s.SCHEDULE_DATE) AS WEEK_END, h.AGENCY_BRANCH_NAME,
    ...
    ROUND(SUM(s.S_ACTUAL_HOURS * 100) / NULLIF(a.AUTH_WEEKLY_LIMIT / 4, 0), 2) as PERCENT_AUTH_USED,
