...
def load_report(report='patients_seen'):
    s = sflake_API()
    if report == 'charts':
        nurses, nonurses = s.charts()
        return pd.json_normalize(nurses), pd.json_normalize(nonurses)
...
def app():
    ...
    if 'chart' not in report_select.lower():
        ...
        gb.configure_column('HOURS', type=['numericColumn', 'numberColumnFilter', 'customNumericFormat'], precision=2)
        ...
        nurses, nonurses = load_report('charts')
        ...
        chart = alt.Chart(billed).encode(x=alt.X('MONTH:O', sort=list(month_map.values()), title=None), y=alt.Y('value', title='Nurse Billable Hours'), color='variable').mark_line()
        chart += alt.Chart(billed).mark_point().encode(x=alt.X('MONTH:O', sort=list(month_map.values()), title=None), y=alt.Y('value', title='Nurse Billable Hours'), color='variable')
        chart2 = alt.Chart(ot).encode(x=alt.X('MONTH:O', sort=list(month_map.values()), title=None), y=alt.Y('value', title='Nurse OT %'), color='variable').mark_line()
        chart2 += alt.Chart(ot).mark_point().encode(x=alt.X('MONTH:O', sort=list(month_map.values()), title=None), y=alt.Y('value', title='Nurse OT %'), color='variable')
